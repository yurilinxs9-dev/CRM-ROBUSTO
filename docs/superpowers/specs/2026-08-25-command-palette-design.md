...
2. **`apps/web/src/components/layout/command-palette.tsx`** — client component, montado UMA vez em `apps/web/src/app/(dashboard)/layout.tsx`:
   ...
   - **Navegação:** itens estáticos com ícone lucide: Dashboard `/dashboard`, Kanban `/kanban`, Leads `/leads`, Conversas `/chat`, Follow-up IA `/followup`, Agenda `/agenda`, Instâncias `/instances`, Configurações `/settings`; Admin `/admin` só quando `user.is_platform_admin` (mesmo campo do sidebar e do guard de `/admin` — role SUPER_ADMIN de tenant NÃO basta; corrigido na review da Task 2). Navegação derivada de `NAV_ITEMS` do sidebar + predicado compartilhado `navVisivelPara`. Filtragem local.
