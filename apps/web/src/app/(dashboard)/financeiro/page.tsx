...
import { isFinanceOwner, canAccessFinance, financeError, financeRequest } from '@/lib/finance';
import { Button } from '@/components/ui/button';
import { FinanceWorkspace } from '@/components/finance/finance-workspace';
const input = 'w-full rounded-md border bg-background px-3 py-2 text-sm';
export default function FinancePage() { const user = useAuthStore(s => s.user); const impersonating = useAuthStore(s => s.impersonating); const owner = useAuthStore(s => isFinanceOwner(s.adminBackup?.user)); if ((impersonating && !owner) || !canAccessFinance(user?.tenantId, user?.id))
...
function FinanceAccess() {
    ...
    useEffect(() => { let live = true; financeRequest<{
        configured: boolean; platform_access?: boolean;
    }>('access').then(r => { if (live)
        { setConfigured(r.configured); setPlatformAccess(r.platform_access === true); } }).catch(e => { if (live)
    ...
    if (platformAccess) return <FinanceWorkspace token="" platformAccess lock={async () => stopImpersonation()} expired={() => { setPlatformAccess(false); setError("Seu acesso de administrador expirou. Entre novamente."); }}/>;
    if (token)
