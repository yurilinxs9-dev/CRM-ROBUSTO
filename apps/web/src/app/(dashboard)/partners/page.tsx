...
import { Plus, RefreshCw, Target, Users, Pencil, History } from 'lucide-react';
...
import { Field, fieldClass, GoalForm, PartnerForm, PartnerModal, ProductionForm } from '@/components/partners/partner-forms';

type Editor = { kind: 'partner'; partner?: Partner; snapshot: PartnerDashboard } | { kind: 'production'; partner: Partner; entry?: Production; date: string; snapshot: PartnerDashboard } | { kind: 'goal'; snapshot: PartnerDashboard };
...
export default function PartnersPage() {
  ...
  return <PartnersWorkspace key={user!.tenantId} tenant={user!.tenantId} role={user!.role} />;
}

function PartnersWorkspace({ tenant, role }: { tenant: string; role: string }) {
  const qc = useQueryClient();
  const [month, setMonth] = useState(() => saoPauloDate().slice(0, 7));
  const [tab, setTab] = useState<'summary' | 'partners' | 'entries' | 'audit'>('summary');
  ...
  async function saved() { setEditor(null); toast.success('Registro salvo.'); await Promise.all([qc.invalidateQueries({ queryKey: ['partners', tenant] }), qc.invalidateQueries({ queryKey: ['partner-audit', tenant] }), qc.invalidateQueries({ queryKey: ['partner-candidates', tenant] })]); }
  ...
  return <main className="mx-auto w-full max-w-[1600px] space-y-6 p-4 sm:p-6 lg:p-8">
    ...
    {data && !query.isError && <>
      ...
      {tab === 'partners' && <PartnerList data={data} manager={manager} edit={p => setEditor({ kind: 'partner', partner: p, snapshot: data })} />}
    ...
    {editor && <PartnerModal close={() => setEditor(null)} title={editor.kind === 'partner' ? editor.partner ? 'Editar parceiro' : 'Cadastrar parceiro' : editor.kind === 'goal' ? 'Meta mensal' : editor.entry ? 'Corrigir vendas do dia' : 'Registrar vendas'} description={editor.kind === 'production' ? 'Um único total por parceiro e dia. O fechamento mensal é automático.' : editor.kind === 'goal' ? 'Defina o volume de vendas que a rede precisa alcançar.' : 'Organize os parceiros que já fazem parte da sua rede.'}>
...
function PartnerList({ data, manager, edit }: { data: PartnerDashboard; manager: boolean; edit: (partner: Partner) => void }) {
