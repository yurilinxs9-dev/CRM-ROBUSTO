  ...
  async getMe(userId: string, tenantId: string) {
    const [user, tenant] = await Promise.all([
      this.prisma.user.findUnique({
        where: { id: userId },
        select: { id: true, nome: true, email: true, role: true, ativo: true, avatar_url: true, titulo: true, especialidade: true, focus_mode: true, is_platform_admin: true, platform_scopes: true },
      }),
