...
it('conta além da página e exporta com o mesmo tenant, visibilidade e período', async () => {
  ...
  await service.exportCsv(user, { ...filters, format: 'xlsx' }, response as unknown as Parameters<LeadsService['exportCsv']>[2]);
  expect(prisma.lead.findMany.mock.calls[2][0].where).toEqual(where);
